...
void treap_delete(Node **node, void *value, int data_size, int (*compar)(void*, void*)) {
	...
	} else if ((*node)->left == NULL) {  // daca am gasit nodul ce trebuie sters, dar nu are fiu stanga
		Node *aux = (*node)->right;
		free((*node)->data);
		free(*node);
		*node = aux;
	} else if ((*node)->right == NULL) {  // daca am gasit nodul ce trebuie sters, dar nu are fiu dreapta
		Node *aux = (*node)->left;
		free((*node)->data);
		free(*node);
		*node = aux;
