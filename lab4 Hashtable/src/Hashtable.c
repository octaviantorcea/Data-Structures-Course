...
void put(struct Hashtable *ht, void *key, int key_size_bytes, void *value) {
    ...
    while (curr != NULL) {
        struct info *aux = curr->data;
        if (ht->compare_function(key, aux->key) == 0) {
            aux->value = value;
            return;
        }
    }

    void *copyKey = malloc(key_size_bytes);
    memcpy(copyKey, key, key_size_bytes);
...
void remove_ht_entry(struct Hashtable *ht, void *key) {
    ...
    while (curr != NULL) {
        ...
        if (ht->compare_function(key, aux->key) == 0) {
            struct Node *curr = remove_nth_node(&ht->buckets[index], count);
            free(aux->key);
            free(aux);
            free(curr);
            ht->size--;
    ...
    printf("Nu s-a gasit elementul de sters\n");
}
